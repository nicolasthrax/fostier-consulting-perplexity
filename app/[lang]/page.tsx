...
export default function HomePage({ params: { lang } }: { params: { lang: "fr" | "en" } }) {
  ...
  return (
    <>
      <section className="relative overflow-hidden bg-gradient-to-b from-parchment via-white to-white">
        ...
        <div className="container-site grid items-center gap-12 py-16 sm:py-20 lg:grid-cols-2 lg:py-24">
          ...
          <figure className="relative overflow-hidden rounded-3xl border border-line bg-gradient-to-br from-white via-parchment to-navy-50 p-6 shadow-soft sm:p-10">
            ...
            <svg viewBox="0 0 500 320" className="relative h-auto w-full" aria-hidden="true"><defs><pattern id="grid" width="24" height="24" patternUnits="userSpaceOnUse"><path d="M24 0H0v24" fill="none" stroke="#E5E7EB" strokeWidth=".7" /></pattern><linearGradient id="route" x1="0" x2="1"><stop offset="0" stopColor="#002395" /><stop offset="1" stopColor="#ED2939" /></linearGradient></defs><rect width="500" height="320" fill="url(#grid)" /><path d="M95 95 C190 24 340 62 408 222" fill="none" stroke="url(#route)" strokeWidth="2" strokeDasharray="5 9" /><path d="M95 95 C230 176 306 198 408 222" fill="none" stroke="#002395" strokeOpacity=".25" strokeWidth="1" /><circle cx="95" cy="95" r="6" fill="#002395" /><circle cx="95" cy="95" r="16" fill="none" stroke="#002395" strokeOpacity=".25" /><circle cx="408" cy="222" r="7" fill="#ED2939" /><circle cx="408" cy="222" r="20" fill="none" stroke="#ED2939" strokeOpacity=".3" /></svg>
      ...
      <section className="bg-white py-20 sm:py-28"><div className="container-site max-w-3xl"><p className="eyebrow">{dict.hongkong.eyebrow}</p><h2 className="h-serif mt-4 text-3xl leading-tight sm:text-4xl">{dict.hongkong.title}</h2>{dict.hongkong.paragraphs.map((p: string) => <p key={p} className="body-lead mt-5">{p}</p>)}</div></section>
    </>
  );
}