...
export function AdvisorMapPill({ locale }: { locale: string }) {
  return (
    <Link
      href={`/${locale}/about`}
      className="focus-ring group absolute bottom-4 left-4 block overflow-hidden rounded-full bg-fred p-[1.5px]"
    >
      <span
        aria-hidden="true"
        ...
        style={{
          background:
            "conic-gradient(from 0deg at 50% 50%, transparent 0 74%, rgba(10,132,255,0) 79%, rgba(10,132,255,1) 88%, rgba(10,132,255,0) 97%, transparent 100%)",
        }}
      />
